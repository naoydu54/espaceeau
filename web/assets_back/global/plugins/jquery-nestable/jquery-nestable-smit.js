        ...
        success: function (data) {
            ...
            if (data.categorysWithoutFile.length > 0) {
                if (data.categorysWithoutFile.length == 1) {
                    var msg = "Cette catégorie parente n'a pas de photo <small>(elle n'apparaîtra pas sur le site)</small> !";
                } else {
                    var msg = "Ces catégories parentes n'ont pas de photo <small>(elles n'apparaîtront pas sur le site)</small> !";
                }
        ...
        success: function (data) {
            ...
            if (data == false) {
                c.html('<div class="alert alert-info alert-dismissable">' +
                    '<h4><i class="fa fa-info-circle"></i> Info</h4> Il n\'y a aucune catégorie pour le moment !' +
                    '</div>');
            } else {
                var jsonUpdate;
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                var html = '<div class="messages-container-category"></div>' +
                                    ...
                                    '<div class="col-md-6">' +
                                    '<input type="text" id="val_title" name="val_title" class="form-control" placeholder="Nom de la catégorie.." value="' + data.category.title + '" required=""/>' +
                                    '</div>' +
                                    '</div>' +
                                    ...
                                    '<div class="col-md-6">' +
                                    '<input type="text" id="val_sub_title" name="val_sub_title" class="form-control" placeholder="Sous-titre de la catégorie.." value="' + data.category.sub_title + '" required=""/>' +
                                    '</div>' +
                                    '</div>';
                                ...
                                if (data.category.sub_id == 0) {
                                    ...
                                    if (data.category.file != 'noFile') {
                                        ...
                                        html += '<div class="form-group">' +
                                            ...
                                            '<h4><i class="fa fa-exclamation-triangle"></i> Attention</h4> Cette catégorie parente n\'a pas de photo !' +
                                            '</div>' +
                                            '</div>';
                                    }
                                ...
                                var dialog = BootstrapDialog.show({
                                    ...
                                    title: '<strong>Editer la catégorie</strong> : ' + data.category.title,
                                    message: html,
                                    buttons: [
                                        {
                                            icon: 'fa fa-ban',
                    ...
                    $('[data-deleteid="' + categoryId + '"]').click(function () {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                var html = '<h4>Voulez vous vraiment supprimer cette catégorie ?</h4>';
                                ...
                                if (data.category.subCategoryCount > 0) {
                                    html += '<p style="margin:0;"><strong>' + data.category.subCategoryCount + '</strong> sous catégorie(s)</p>';
                                }
                                ...
                                if (data.category.subId == 0) {
                                    var title = '<strong>Supprimer la catégorie</strong> : ' + data.category.title;
                                } else {
                                    var title = '<strong>Supprimer la sous catégorie</strong> : ' + data.category.title;
                                }
